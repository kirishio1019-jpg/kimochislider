-- 重複しているデフォルト地図を1つにまとめるSQL
-- SupabaseのSQL Editorで実行してください

-- ステップ1: 各コミュニティのデフォルト地図を確認
SELECT 
...
-- ステップ2: 各コミュニティで最も古いデフォルト地図を1つ残し、残りを削除
-- 注意: この操作は不可逆です。実行前に上記のSELECTで確認してください

-- 各コミュニティで最も古いデフォルト地図のIDを取得し、それ以外を削除
...
-- ステップ3: 確認（各コミュニティにデフォルト地図が1つだけあることを確認）
SELECT 
  c.id as community_id,
  c.name as community_name,
